...
import { SignProgress, SignProgressFn, signUp, WebAuthnSignUpProgressStep } from '@junobuild/core';
...
export class CreatePasskeyComponent {
  ...
  wizardOnProgress = input.required<(progress: PasskeyProgress | undefined) => void>();

  @ViewChild('inputText') readonly inputText: ElementRef<HTMLInputElement> | undefined;
  passkeyDisplayName = signal<string>('');

  readonly #computedProgress = computed<ProgressSignUp>(() => {
    const progress = this.wizardProgress();

    return progress === undefined ? { state: "init" } : (
      "signUp" in progress
