...
test('get quickstart link', async ({ page }) => {
  ...
  await expect(link).toHaveAttribute('href', 'https://juno.build/docs/guides/astro');
});

test('get documentation link', async ({ page }) => {
  await page.goto('/');

  const link = page.locator('a[aria-label="Open the list of features for building apps on Juno\'s website"]');
...
test('get ci link', async ({ page }) => {
  ...
  await expect(link).toHaveAttribute('href', 'https://juno.build/docs/miscellaneous/github_actions');
});

test('get discord link', async ({ page }) => {
  await page.goto('/');

  const link = page.locator('a[aria-label="Join Juno\'s Discord channel for questions or to share the fun"]');
